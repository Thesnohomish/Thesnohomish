...
export type DbHomepageSection = { id: string; heading: string; description?: string | null; category_id?: string | null; product_ids: string[]; use_best_sellers: boolean; item_limit: number; sort_order: number; is_active: boolean; updated_at?: string; categories?: { slug: string } | null };
...
async function supabaseFetch<T>(path: string, options: SupabaseFetchOptions = {}): Promise<T[]> {
  if (!hasSupabaseConfig) {
    console.error('[The Snohomish Supabase] Configuration is missing; returning an empty public result.', { resource: options.resource || path });
    return [];
  }
  try {
    ...
    if (!response.ok) {
      ...
      console.error('[The Snohomish Supabase]', error.message, { path, project: getSupabaseProjectRef() });
      return [];
    }
    return response.json();
  } catch (error) {
    console.error(`[The Snohomish Supabase] ${options.resource || 'request'} failed`, { path, project: getSupabaseProjectRef(), error });
    return [];
  }
}
...
export async function getBanners(): Promise<DbBanner[]> {
  ...
  console.info('[The Snohomish banners] Supabase synchronization complete', { project: getSupabaseProjectRef(), fetched: rows.length, visible: activeRows.length });
  return activeRows;
}
