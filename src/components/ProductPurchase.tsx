...
export function ProductPurchase({ product, initialVariantId }: { product: DbProduct; initialVariantId?: string }) {
  ...
  const rewardPoints = Math.floor((price * quantity) / 100);
  ...
  return <div className="mt-4">
    ...
    <div className="mt-3 rounded-xl border border-yellow-300 bg-yellow-50 px-3 py-2.5 text-sm text-brand-ink"><p className="font-black">Earn {rewardPoints.toLocaleString('en-KE')} reward points</p><p className="mt-0.5 text-xs font-medium text-neutral-700">Worth KSh {rewardPoints.toLocaleString('en-KE')} after delivery · Every KSh 1,000 earns 10 points.</p><Link href="/login" className="mt-2 inline-flex text-xs font-black text-brand-orange underline underline-offset-2">Sign in to save your points</Link></div>
