...
export function ProductPurchase({ product, initialVariantId }: { product: DbProduct; initialVariantId?: string }) {
  ...
  return <div className="mt-4">
    {variants.length > 0 && <fieldset><legend className="text-sm font-black text-brand-ink">Available bottle sizes</legend><p className="mt-1 text-xs text-neutral-600">Choose a size before adding this product to your cart.</p><div className="mt-2 flex flex-wrap gap-2">{variants.map((variant) => { const optionPrice = effectivePrice(variant); return <button key={variant.id} type="button" onClick={() => { setSelectedId(variant.id); setQuantity(1); }} disabled={variant.stock <= 0} className={`min-w-20 rounded-lg border px-3 py-2 text-left text-sm font-bold leading-tight transition ${selectedId === variant.id ? 'border-brand-orange bg-orange-50 text-brand-deep' : 'border-orange-100 bg-white text-neutral-700'} disabled:cursor-not-allowed disabled:opacity-45`}><span className="block">{variant.name}</span><span className="mt-0.5 block text-xs font-semibold">{money(optionPrice.price)}</span>{variant.stock <= 0 && <span className="mt-1 block text-[10px] font-bold text-red-600">Out of stock</span>}</button>})}</div></fieldset>}
