...
export function ProductPurchase({ product, initialVariantId }: { product: DbProduct; initialVariantId?: string }) {
  ...
  return <div className="mt-4">
    ...
    <button type="button" onClick={addToCart} disabled={!available} className="orange-gradient mt-4 min-h-12 w-full touch-manipulation rounded-xl px-4 py-3 text-center text-sm font-black leading-snug text-white shadow-orange disabled:cursor-not-allowed disabled:opacity-50 sm:px-6 sm:text-base">{available ? addToCartLabel : 'Unavailable'}</button>
  </div>;
}
