...
export function LiveOrdersNav() {
  ...
  const refresh = useCallback(async () => {
    ...
    const { count: total } = await supabase
      ...
      .in("status", ["pending", "pending_payment", "paid"]);
    setCount(total || 0);
  }, [supabase]);
  useEffect(() => {
    if (!supabase) return;
  ...
  return (
    <>
      <a
        href="/admin/orders"
        className="flex shrink-0 items-center gap-2 rounded-xl px-4 py-3 text-left font-bold hover:bg-orange-50 lg:mb-1 lg:w-full"
      >
