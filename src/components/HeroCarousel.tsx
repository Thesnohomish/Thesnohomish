...
export function HeroCarousel({ banners }: { banners: DbBanner[] }) {
  ...
  return <section onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onTouchStart={() => setPaused(true)} onTouchEnd={() => setPaused(false)} aria-roledescription="carousel" aria-label="The Snohomish promotions" className="hero-carousel relative mx-auto max-w-none overflow-hidden bg-neutral-900 shadow-card">
    <div className="relative h-[230px] w-full sm:h-[300px] lg:h-[360px]">{slides.map((banner,index) => <article key={banner.id} aria-hidden={index !== current} className={`absolute inset-0 transition-opacity duration-700 ${index === current ? 'z-10 opacity-100' : 'pointer-events-none opacity-0'}`}><SmartImage src={banner.mobile_image_url || banner.image_url} alt={banner.title || 'The Snohomish promotion'} sizes="100vw" priority={index === 0} quality={92} className="scale-[1.01]"/><div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/35 to-transparent"/><div className="absolute inset-y-0 left-0 z-10 flex max-w-2xl flex-col justify-center px-12 py-8 text-white sm:px-20 lg:px-24"><h1 className="text-2xl font-black sm:text-4xl">{banner.title}</h1>{banner.subtitle&&<p className="mt-2 max-w-xl text-sm leading-6 text-white/90 sm:text-base">{banner.subtitle}</p>}{banner.button_url&&<Link href={banner.button_url} className="mt-4 w-fit rounded-full bg-brand-orange px-5 py-2.5 text-xs font-black uppercase tracking-wide text-white">{banner.button_label||banner.button_text||'Shop now'}</Link>}</div></article>)}</div>
