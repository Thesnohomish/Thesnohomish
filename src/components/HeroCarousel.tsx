...
export function HeroCarousel({ banners }: { banners: DbBanner[] }) {
  ...
  return <section onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onTouchStart={() => setPaused(true)} onTouchEnd={() => setPaused(false)} aria-roledescription="carousel" aria-label="The Snohomish promotions" className="relative mx-auto max-w-none overflow-hidden bg-neutral-100 shadow-card sm:mt-4 sm:rounded-3xl">
    <div className="relative aspect-[4/3] min-h-56 w-full sm:aspect-[16/7] lg:aspect-[21/8]">{slides.map((banner,index) => <article key={banner.id} aria-hidden={index !== current} className={`absolute inset-0 transition-opacity duration-700 ${index === current ? 'z-10 opacity-100' : 'pointer-events-none opacity-0'}`}>{banner.button_url?<Link href={banner.button_url} aria-label={banner.title || 'Open promotion'} className="absolute inset-0"><SmartImage src={banner.mobile_image_url || banner.image_url} alt={banner.title || 'The Snohomish promotion'} sizes="100vw" priority={index === 0} quality={92} className="scale-[1.01]" /></Link>:<SmartImage src={banner.mobile_image_url || banner.image_url} alt={banner.title || 'The Snohomish promotion'} sizes="100vw" priority={index === 0} quality={92} className="scale-[1.01]" />}</article>)}</div>
