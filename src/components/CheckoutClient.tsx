...
type PaymentMethod = 'mpesa' | 'cash' | 'pickup';
type CheckoutDraft = { name?: string; email?: string; phone?: string; address?: string; apartment?: string; building?: string; instructions?: string; coordinates?: DeliveryLocation | null; manualLocation?: boolean; gift?: string; payment?: PaymentMethod };
...
function lastItem<T>(items: T[]): T | null { return items.length ? items[items.length - 1] : null; }
function deliveryFor(distance: number | null, bands: DbDeliverySetting[]) { if (distance == null) return null; return bands.find((band) => distance >= Number(band.min_distance_km) && (band.max_distance_km == null || distance <= Number(band.max_distance_km))) || lastItem(bands); }

export function CheckoutClient({ settings, bands }: { settings: CheckoutSettings; bands: DbDeliverySetting[] }) {
  const [items, setItems] = useState<CartItem[]>([]), [name, setName] = useState(''), [email, setEmail] = useState(''), [phone, setPhone] = useState(''), [address, setAddress] = useState(''), [apartment, setApartment] = useState(''), [building, setBuilding] = useState(''), [instructions, setInstructions] = useState(''), [manualLocation, setManualLocation] = useState(false), [gift, setGift] = useState(''), [payment, setPayment] = useState<PaymentMethod>('mpesa'), [notice, setNotice] = useState(''), [error, setError] = useState(''), [submitting, setSubmitting] = useState(false), [signedIn, setSignedIn] = useState(false), [authError, setAuthError] = useState('');
  ...
  const productTotal = items.reduce((sum, item) => sum + item.price * item.quantity, 0); const storeLatitude = Number(settings.store_latitude ?? process.env.NEXT_PUBLIC_STORE_LATITUDE), storeLongitude = Number(settings.store_longitude ?? process.env.NEXT_PUBLIC_STORE_LONGITUDE), storeConfigured = Number.isFinite(storeLatitude) && Number.isFinite(storeLongitude); const km = coordinates && storeConfigured ? distanceKm(storeLatitude, storeLongitude, coordinates.latitude, coordinates.longitude) : null; const band = useMemo(() => manualLocation ? lastItem(bands) : deliveryFor(km, bands), [manualLocation, km, bands]); const freeDelivery = productTotal >= 10000; const delivery = payment === 'pickup' || freeDelivery ? 0 : Number(band?.fee || 0);
  const methods = useMemo(() => [{ id: 'mpesa' as const, label: 'M-Pesa', on: settings.allow_mpesa !== false }, { id: 'cash' as const, label: 'Cash on delivery', on: settings.allow_cash !== false }, { id: 'pickup' as const, label: 'Store pickup', on: true }].filter((method) => method.on), [settings.allow_cash, settings.allow_mpesa]);
  useEffect(() => { if (!methods.some((method) => method.id === payment)) setPayment(methods[0]?.id || 'pickup'); }, [methods, payment]);
