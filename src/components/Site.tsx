...
function CatalogCards({ products, limit, uniqueProducts = false }: { products: DbProduct[]; limit?: number; uniqueProducts?: boolean }) {
  if (uniqueProducts) return <>{products.slice(0, limit).map(product => <ProductCard key={product.id} p={product} />)}</>;
...
export function ProductRail({ title, products, href, limit = 8, uniqueProducts = false }: { title: string; products: DbProduct[]; href: string; limit?: number; uniqueProducts?: boolean }) {
