...
export function ProductCard({ p }: { p: DbProduct }) {
  ...
  return <Link href={`/product/${p.slug}`} className="flex h-full min-w-0 flex-col rounded-xl bg-white p-1.5 transition hover:-translate-y-1 hover:shadow-card"><div className="relative"><ProductImage src={imageFor(p)} alt={`${p.name} product image`} /><button type="button" aria-label={`Add ${p.name} to cart`} onClick={add} className="absolute right-2 top-1/2 flex min-h-10 -translate-y-1/2 items-center gap-1 rounded-full bg-brand-orange px-3 text-xs font-black text-white shadow-orange transition hover:scale-105 disabled:bg-neutral-300" disabled={!available || adding}>{adding ? <span className="h-4 w-4 animate-spin rounded-full border-2 border-white border-r-transparent"/> : available ? <><ShoppingCart size={18} strokeWidth={2.6}/><span>ADD</span></> : 'OUT'}</button>{variants.length > 1 && <span className="absolute bottom-2 left-2 rounded-full bg-white/95 px-2 py-1 text-[10px] font-black text-brand-deep shadow-sm">{variants.length} sizes</span>}</div><div className="flex flex-1 flex-col pt-3"><div className="flex flex-wrap items-center gap-2"><b className="rounded-md bg-brand-orange px-1.5 py-1 text-sm leading-none text-white"><span className="text-[9px] text-white">KSh</span> {Number(price).toLocaleString('en-KE')}</b>{discount > 0 && <span className="text-xs font-bold text-brand-deep">{discount}% off</span>}{oldPrice && <s className="text-sm font-semibold text-red-600 decoration-red-600 decoration-2">{money(oldPrice)}</s>}</div><h3 className="mt-1.5 line-clamp-2 min-h-10 text-sm font-bold leading-snug text-neutral-900">{p.name}</h3><ProductCardDetails country={p.country} abv={p.abv} size={firstVariant?.name || p.bottle_size} available={available}/></div></Link>;
}
...
export function ProductVariantCard({ product, variant }: { product: DbProduct; variant: NonNullable<DbProduct['product_variants']>[number] }) {
  ...
  return <Link href={`/product/${product.slug}?variant=${encodeURIComponent(variant.id)}`} className="flex h-full min-w-0 flex-col rounded-xl bg-white p-1.5 transition hover:-translate-y-1 hover:shadow-card"><div className="relative"><ProductImage src={variant.image_url || imageFor(product)} alt={`${product.name} ${variant.name} product image`} /><button type="button" aria-label={`Add ${product.name} ${variant.name} to cart`} onClick={add} disabled={!available || adding} className="absolute right-2 top-1/2 flex min-h-10 -translate-y-1/2 items-center gap-1 rounded-full bg-brand-orange px-3 text-xs font-black text-white shadow-orange transition hover:scale-105 disabled:bg-neutral-300">{adding ? <span className="h-4 w-4 animate-spin rounded-full border-2 border-white border-r-transparent"/> : available ? <><ShoppingCart size={18} strokeWidth={2.6}/><span>ADD</span></> : 'OUT'}</button></div><div className="flex flex-1 flex-col pt-3"><div className="flex flex-wrap items-center gap-2"><b className="rounded-md bg-brand-orange px-1.5 py-1 text-sm leading-none text-white"><span className="text-[9px] text-white">KSh</span> {Number(pricing.price).toLocaleString('en-KE')}</b>{discount > 0 && <span className="text-xs font-bold text-brand-deep">{discount}% off</span>}{oldPrice && <s className="text-sm font-semibold text-red-600 decoration-red-600 decoration-2">{money(oldPrice)}</s>}</div><h3 className="mt-1.5 line-clamp-2 min-h-10 text-sm font-bold leading-snug text-neutral-900">{product.name}</h3><ProductCardDetails country={product.country} abv={product.abv} size={variant.name} available={available}/></div></Link>;
}
