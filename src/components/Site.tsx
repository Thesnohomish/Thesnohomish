...
import { Heart, Menu, Search, ShoppingBag, ShoppingCart, UserCircle } from 'lucide-react';
...
export function Header({ content = {}, products = [] }: { content?: SiteContent; products?: DbProduct[] }) {
  const [cart, setCart] = useState<{ count: number; total: number }>({ count: 0, total: 0 }), [query, setQuery] = useState(''), [menuOpen, setMenuOpen] = useState(false), [customerName, setCustomerName] = useState('');
  ...
  useEffect(() => {
    ...
    const applyUser = (user: { email?: string; user_metadata?: Record<string, unknown> } | null) => {
      ...
      const fullName = String(metadata.full_name || metadata.name || user?.email?.split('@')[0] || '').trim();
  ...
  const links = [['Retail Stores','/#stores'],['Wholesale','/#wholesale'],['Deals','/offers'],['Mega Drive-Through','/#mega-drive-through'],['About','/about'],['Contact','/contact']];
  const categoryLinks = [...new Map(products.filter(product => product.categories?.slug).map(product => [product.categories!.slug, product.categories!.name])).entries()].sort((a,b)=>a[1].localeCompare(b[1]));
  const savedAnnouncement = content.header_notice?.trim();
  const announcement = !savedAnnouncement || savedAnnouncement === 'Reliable delivery across Nairobi'
    ? 'Ordering for business or corporate? Call +254 726 764 759'
    : savedAnnouncement;
  ...
  return <header className="sno-header sticky top-0 z-40">
    <div className="announcement"><span>{announcement}</span></div>
    <div className="mx-auto flex max-w-7xl items-center gap-3 px-4 py-3 md:px-8"><button className="menu-button lg:hidden" onClick={()=>setMenuOpen(v=>!v)} aria-expanded={menuOpen} aria-label="Open navigation"><Menu/></button><Link href="/" aria-label="The Snohomish home"><BrandLogo/></Link><nav className="ml-4 hidden items-center gap-x-4 whitespace-nowrap text-xs font-bold text-white/80 lg:flex xl:gap-x-5"><span className="group relative"><button className="hover:text-sno-yellow">Categories ▾</button><span className="invisible absolute left-0 top-full z-50 mt-2 grid min-w-52 gap-1 rounded-xl border border-white/10 bg-[#191919] p-2 opacity-0 shadow-2xl transition group-hover:visible group-hover:opacity-100 group-focus-within:visible group-focus-within:opacity-100">{categoryLinks.map(([slug,name])=><Link key={slug} href={`/category/${slug}`} className="rounded-lg px-3 py-2 text-white hover:bg-white/10 hover:text-sno-yellow">{name}</Link>)}</span></span>{links.map(([label,href])=><Link key={href} href={href} className="hover:text-sno-yellow">{label}</Link>)}</nav><div className="relative ml-auto hidden w-full max-w-sm md:block"><Search className="absolute left-3 top-3 text-sno-yellow" size={18}/><input value={query} onChange={e=>setQuery(e.target.value)} className="header-search" placeholder="Search wines, spirits, beers, mixers and brands..." aria-label="Search products"/>{suggestions.length>0&&<div className="search-results">{suggestions.map(p=><Link key={p.id} href={`/product/${p.slug}`} onClick={()=>setQuery('')}><img src={imageFor(p)} alt=""/><span><b>{p.name}</b><small>{money(p.product_variants?.[0]?.price??p.price)}</small></span></Link>)}</div>}</div><div className="header-icons"><Link href="/wishlist" aria-label="Wishlist"><Heart/></Link><Link href={customerName ? "/account" : "/login"} aria-label={customerName ? `${customerName}'s account` : "Customer login"} className="flex max-w-28 items-center gap-1.5 text-xs font-bold text-white"><UserCircle/><span className="truncate">{customerName || 'Login'}</span></Link><Link href="/checkout" data-cart-icon className="cart-link" aria-label={`Cart with ${cart.count} items`}><ShoppingBag/><b>{cart.count}</b><span>{money(cart.total)}</span></Link></div></div>
    ...
    {menuOpen&&<nav className="mobile-nav"><details><summary className="cursor-pointer py-3 font-bold">Categories</summary><span className="grid pl-3">{categoryLinks.map(([slug,name])=><Link onClick={()=>setMenuOpen(false)} key={slug} href={`/category/${slug}`}>{name}</Link>)}</span></details>{links.map(([label,href])=><Link onClick={()=>setMenuOpen(false)} key={href} href={href}>{label}</Link>)}</nav>}
  </header>;
}
...
export function CategoryGrid({ categories }: { categories: DbCategory[] }) {
  return <section className="mx-auto grid grid-cols-3 gap-2 px-3 py-4 sm:grid-cols-4 sm:px-4 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-12">{categories.map((category) => <Link href={`/category/${category.slug}`} key={category.id} className="group relative aspect-[4/3] overflow-hidden rounded-lg bg-neutral-100 shadow-sm ring-1 ring-black/5 transition hover:-translate-y-0.5 hover:shadow-card"><SmartImage src={category.image_url || 'https://images.unsplash.com/photo-1514362545857-3bc16c4c7d1b?auto=format&fit=crop&w=700&q=80'} alt={`${category.name} category`} sizes="(max-width: 640px) 33vw, (max-width: 768px) 25vw, (max-width: 1024px) 17vw, (max-width: 1280px) 13vw, 9vw" fit="contain" quality={95} className="p-1 transition duration-500 group-hover:scale-[1.02]" /><div className="absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-t from-black/75 to-transparent" /><div className="absolute inset-x-0 bottom-0 px-2 py-1.5 text-white"><h2 className="truncate text-[11px] font-semibold tracking-wide sm:text-xs">{category.name}</h2></div></Link>)}</section>;
}

function ProductCardDetails({ country, abv, size, available }: { country?: string; abv?: number; size?: string; available: boolean }) {
  return <div className="mt-1 space-y-0.5 text-[10px] font-medium uppercase leading-tight tracking-wide text-neutral-700"><p className="truncate"><span className="font-semibold text-neutral-500">Origin</span> {country || 'Not specified'}</p><p className="truncate"><span className="font-semibold text-neutral-500">ABV</span> {abv != null ? `${abv}%` : 'Not specified'} · <span className="font-semibold text-neutral-500">Size</span> {size || 'Not specified'}</p><p className={available ? 'font-semibold text-emerald-700' : 'font-semibold text-red-600'}>{available ? 'Available' : 'Out of stock'}</p></div>;
}

export function ProductCard({ p }: { p: DbProduct }) {
  const [adding, setAdding] = useState(false);
  const variants = (p.product_variants || []).filter((variant) => variant.is_active !== false);
  const firstVariant = variants.find((variant) => Number(variant.stock) > 0) || variants[0], pricing = effectivePrice(firstVariant || p), price = pricing.price, oldPrice = pricing.oldPrice;
  ...
  return <Link href={`/product/${p.slug}`} className="flex h-full min-w-0 flex-col rounded-xl bg-white p-1.5 transition hover:-translate-y-1 hover:shadow-card"><div className="relative"><ProductImage src={imageFor(p)} alt={`${p.name} product image`} /><button type="button" aria-label={`Add ${p.name} to cart`} onClick={add} className="absolute right-2 top-2 flex min-h-9 items-center gap-1 rounded-full bg-brand-orange px-2.5 text-[10px] font-extrabold leading-none text-white shadow-orange transition hover:scale-105 disabled:bg-neutral-300" disabled={!available || adding}>{adding ? <span className="h-4 w-4 animate-spin rounded-full border-2 border-white border-r-transparent"/> : available ? <><ShoppingCart size={16} strokeWidth={2.5}/><span>ADD</span></> : 'OUT'}</button>{variants.length > 1 && <span className="absolute bottom-2 left-2 rounded-full bg-white/95 px-2 py-1 text-[10px] font-black text-brand-deep shadow-sm">{variants.length} sizes</span>}</div><div className="flex flex-1 flex-col pt-3"><div className="flex flex-wrap items-center gap-2"><b className="rounded-md bg-brand-orange px-1.5 py-1 text-sm leading-none text-white"><span className="text-[9px] text-white">KSh</span> {Number(price).toLocaleString('en-KE')}</b>{discount > 0 && <span className="text-xs font-bold text-brand-deep">{discount}% off</span>}{oldPrice && <s className="text-sm font-semibold text-red-600 decoration-red-600 decoration-2">{money(oldPrice)}</s>}</div><h3 className="mt-1.5 line-clamp-2 min-h-10 text-sm font-bold leading-snug text-neutral-900">{p.name}</h3><ProductCardDetails country={p.country} abv={p.abv} size={firstVariant?.name || p.bottle_size} available={available}/></div></Link>;
}
...
export function ProductVariantCard({ product, variant }: { product: DbProduct; variant: NonNullable<DbProduct['product_variants']>[number] }) {
  ...
  return <Link href={`/product/${product.slug}?variant=${encodeURIComponent(variant.id)}`} className="flex h-full min-w-0 flex-col rounded-xl bg-white p-1.5 transition hover:-translate-y-1 hover:shadow-card"><div className="relative"><ProductImage src={variant.image_url || imageFor(product)} alt={`${product.name} ${variant.name} product image`} /><button type="button" aria-label={`Add ${product.name} ${variant.name} to cart`} onClick={add} disabled={!available || adding} className="absolute right-2 top-2 flex min-h-9 items-center gap-1 rounded-full bg-brand-orange px-2.5 text-[10px] font-extrabold leading-none text-white shadow-orange transition hover:scale-105 disabled:bg-neutral-300">{adding ? <span className="h-4 w-4 animate-spin rounded-full border-2 border-white border-r-transparent"/> : available ? <><ShoppingCart size={16} strokeWidth={2.5}/><span>ADD</span></> : 'OUT'}</button></div><div className="flex flex-1 flex-col pt-3"><div className="flex flex-wrap items-center gap-2"><b className="rounded-md bg-brand-orange px-1.5 py-1 text-sm leading-none text-white"><span className="text-[9px] text-white">KSh</span> {Number(pricing.price).toLocaleString('en-KE')}</b>{discount > 0 && <span className="text-xs font-bold text-brand-deep">{discount}% off</span>}{oldPrice && <s className="text-sm font-semibold text-red-600 decoration-red-600 decoration-2">{money(oldPrice)}</s>}</div><h3 className="mt-1.5 line-clamp-2 min-h-10 text-sm font-bold leading-snug text-neutral-900">{product.name}</h3><ProductCardDetails country={product.country} abv={product.abv} size={variant.name} available={available}/></div></Link>;
}
...
export function ProductRail({ title, description, products, href, limit = 18 }: { title: string; description?: string; products: DbProduct[]; href: string; limit?: number }) {
  const shouldCarousel = products.length > 0;
  const rail = useRef<HTMLDivElement>(null);
  const paused = useRef(false);
  useEffect(()=>{if(!shouldCarousel||window.matchMedia('(prefers-reduced-motion: reduce)').matches||window.matchMedia('(max-width: 639px)').matches)return;let frame=0,last=performance.now();const tick=(now:number)=>{const node=rail.current,elapsed=Math.min(now-last,50);last=now;if(node&&!paused.current&&node.scrollWidth>node.clientWidth){node.scrollLeft+=elapsed*.018;const halfway=node.scrollWidth/2;if(node.scrollLeft>=halfway)node.scrollLeft-=halfway}frame=requestAnimationFrame(tick)};frame=requestAnimationFrame(tick);return()=>cancelAnimationFrame(frame)},[shouldCarousel]);
  const move = (direction:number) => rail.current?.scrollBy({left:direction*rail.current.clientWidth,behavior:'smooth'});
  return <section className="mx-auto max-w-none overflow-hidden px-3 py-5 sm:px-6 lg:px-8"><div className="mb-4 flex items-end justify-between gap-3"><div><h2 className="text-2xl font-extrabold tracking-tight text-brand-ink">{title}</h2>{description&&<p className="mt-1 max-w-2xl text-sm leading-5 text-neutral-600">{description}</p>}</div><Link href={href} className="shrink-0 font-bold text-brand-orange">View all</Link></div>{shouldCarousel?<div className="product-rail-frame" onMouseEnter={()=>{paused.current=true}} onMouseLeave={()=>{paused.current=false}} onFocusCapture={()=>{paused.current=true}} onBlurCapture={()=>{paused.current=false}}><button type="button" onClick={()=>move(-1)} className="product-rail-arrow left-1" aria-label={`Scroll ${title} left`}>‹</button><div ref={rail} className="product-rail-scroll"><div className="product-rail-loop"><div className="product-rail-loop-group"><CatalogCards products={products} limit={limit}/></div><div className="product-rail-loop-group" aria-hidden="true" inert><CatalogCards products={products} limit={limit}/></div></div></div><button type="button" onClick={()=>move(1)} className="product-rail-arrow right-1" aria-label={`Scroll ${title} right`}>›</button></div>:<div className="product-rail-static"><CatalogCards products={products} limit={9}/></div>}</section>;
}
