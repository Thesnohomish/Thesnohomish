...
export function ProductRail({ title, products, href, limit = 18 }: { title: string; products: DbProduct[]; href: string; limit?: number }) {
  ...
  const availableCards = products.reduce((count, product) => count + Math.max(1, (product.product_variants || []).filter(variant => variant.is_active !== false).length), 0);
  const visibleCards = Math.max(1, Math.min(limit, availableCards));
  const railStyle = { '--product-rail-duration': `${visibleCards * 5}s` } as React.CSSProperties;
  return <section className="mx-auto max-w-none overflow-hidden px-6 py-8"><div className="mb-4 flex items-center justify-between"><h2 className="text-xl font-extrabold tracking-tight text-brand-ink">{title}</h2><Link href={href} className="font-bold text-brand-orange">View all {title}</Link></div>{shouldCarousel?<div className="product-rail-grid"><div className="product-rail-track" style={railStyle}><div className="product-rail-group"><CatalogCards products={products} limit={limit} /></div><div className="product-rail-group" aria-hidden="true" inert><CatalogCards products={products} limit={limit} /></div></div></div>:<div className="product-rail-static"><CatalogCards products={products} limit={limit}/></div>}</section>;
}
