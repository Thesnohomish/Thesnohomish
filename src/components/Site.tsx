...
export function Header({ content = {}, categories = [] }: { content?: SiteContent; categories?: DbCategory[] }) {
  const [cart, setCart] = useState<{ count: number; total: number }>({ count: 0, total: 0 }), [menuOpen, setMenuOpen] = useState(false), [customerName, setCustomerName] = useState('');
  ...
  const categoryLinks = categories.map(category => [category.slug, category.name] as const).sort((a,b)=>a[1].localeCompare(b[1]));
...
export function ProductCard({ p }: { p: DbProduct }) {
  ...
  return <article className="relative flex h-full min-w-0 flex-col rounded-xl bg-white p-1.5 transition hover:-translate-y-1 hover:shadow-card"><Link href={`/product/${p.slug}`} aria-label={`View ${p.name}`} className="absolute inset-0 z-0 rounded-xl"/><div className="pointer-events-none relative z-10"><ProductImage src={imageFor(p)} alt={`${p.name} product image`} /><button type="button" aria-label={`Add ${p.name} to cart`} onClick={add} className="pointer-events-auto absolute right-2 top-2 flex min-h-9 items-center gap-1 rounded-full bg-brand-orange px-2.5 text-[10px] font-extrabold leading-none text-white shadow-orange transition hover:scale-105 disabled:bg-neutral-300" disabled={!available || adding}>{adding ? <span className="h-4 w-4 animate-spin rounded-full border-2 border-white border-r-transparent"/> : available ? <><ShoppingCart size={16} strokeWidth={2.5}/><span>ADD</span></> : 'OUT'}</button>{variants.length > 1 && <span className="absolute bottom-2 left-2 rounded-full bg-white/95 px-2 py-1 text-[10px] font-black text-brand-deep shadow-sm">{variants.length} sizes</span>}</div><div className="pointer-events-none relative z-10 flex flex-1 flex-col pt-3"><div className="flex flex-wrap items-center gap-2"><b className="rounded-md bg-brand-orange px-1.5 py-1 text-sm leading-none text-white"><span className="text-[9px] text-white">KSh</span> {Number(price).toLocaleString('en-KE')}</b>{discount > 0 && <span className="text-xs font-bold text-brand-deep">{discount}% off</span>}{oldPrice && <s className="text-sm font-semibold text-red-600 decoration-red-600 decoration-2">{money(oldPrice)}</s>}</div><h3 className="mt-1.5 line-clamp-2 min-h-10 text-sm font-bold leading-snug text-neutral-900">{p.name}</h3><ProductCardDetails country={p.country} abv={p.abv} size={firstVariant?.name || p.bottle_size} available={available}/></div></article>;
}
...
export function ProductVariantCard({ product, variant }: { product: DbProduct; variant: NonNullable<DbProduct['product_variants']>[number] }) {
  ...
  return <article className="relative flex h-full min-w-0 flex-col rounded-xl bg-white p-1.5 transition hover:-translate-y-1 hover:shadow-card"><Link href={`/product/${product.slug}?variant=${encodeURIComponent(variant.id)}`} aria-label={`View ${product.name} ${variant.name}`} className="absolute inset-0 z-0 rounded-xl"/><div className="pointer-events-none relative z-10"><ProductImage src={variant.image_url || imageFor(product)} alt={`${product.name} ${variant.name} product image`} /><button type="button" aria-label={`Add ${product.name} ${variant.name} to cart`} onClick={add} disabled={!available || adding} className="pointer-events-auto absolute right-2 top-2 flex min-h-9 items-center gap-1 rounded-full bg-brand-orange px-2.5 text-[10px] font-extrabold leading-none text-white shadow-orange transition hover:scale-105 disabled:bg-neutral-300">{adding ? <span className="h-4 w-4 animate-spin rounded-full border-2 border-white border-r-transparent"/> : available ? <><ShoppingCart size={16} strokeWidth={2.5}/><span>ADD</span></> : 'OUT'}</button></div><div className="pointer-events-none relative z-10 flex flex-1 flex-col pt-3"><div className="flex flex-wrap items-center gap-2"><b className="rounded-md bg-brand-orange px-1.5 py-1 text-sm leading-none text-white"><span className="text-[9px] text-white">KSh</span> {Number(pricing.price).toLocaleString('en-KE')}</b>{discount > 0 && <span className="text-xs font-bold text-brand-deep">{discount}% off</span>}{oldPrice && <s className="text-sm font-semibold text-red-600 decoration-red-600 decoration-2">{money(oldPrice)}</s>}</div><h3 className="mt-1.5 line-clamp-2 min-h-10 text-sm font-bold leading-snug text-neutral-900">{product.name}</h3><ProductCardDetails country={product.country} abv={product.abv} size={variant.name} available={available}/></div></article>;
}
...
export function ProductRail({ title, description, products, href, limit = 18 }: { title: string; description?: string; products: DbProduct[]; href: string; limit?: number }) {
  ...
  return <section className="deferred-section mx-auto max-w-none overflow-hidden px-3 py-5 sm:px-6 lg:px-8"><div className="mb-4 flex items-end justify-between gap-3"><div><h2 className="text-2xl font-extrabold tracking-tight text-brand-ink">{title}</h2>{description&&<p className="mt-1 max-w-2xl text-sm leading-5 text-neutral-600">{description}</p>}</div><Link href={href} className="shrink-0 font-bold text-brand-orange">View all</Link></div>{shouldCarousel?<div className="product-rail-frame"><button type="button" onClick={()=>move(-1)} className="product-rail-arrow left-1" aria-label={`Scroll ${title} left`}>‹</button><div ref={rail} className="product-rail-scroll"><div className="product-rail-loop"><div className="product-rail-loop-group"><CatalogCards products={products} limit={limit}/></div></div></div><button type="button" onClick={()=>move(1)} className="product-rail-arrow right-1" aria-label={`Scroll ${title} right`}>›</button></div>:<div className="product-rail-static"><CatalogCards products={products} limit={9}/></div>}</section>;
}
