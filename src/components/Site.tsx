...
export function Header({ content = {}, products = [] }: { content?: SiteContent; products?: DbProduct[] }) {
  ...
  const announcement = content.header_notice || 'To become a partner or a distributor, please reach us on 0726764759';
...
export function ProductCard({ p }: { p: DbProduct }) {
  ...
  return <Link href={`/product/${p.slug}`} className="block min-w-0 rounded-xl bg-white p-1.5 transition hover:-translate-y-1 hover:shadow-card"><div className="relative aspect-[3/4] overflow-hidden rounded-lg bg-white"><SmartImage src={imageFor(p)} alt={`${p.name} product image`} sizes="(max-width: 640px) 36vw, (max-width: 1024px) 15vw, 10vw" fit="contain" quality={95} className="scale-[1.06] transition-transform duration-300 hover:scale-110" /><button type="button" aria-label={`Add ${p.name} to cart`} onClick={add} className="absolute right-1.5 top-1.5 grid h-8 w-8 place-items-center rounded-full bg-brand-orange text-lg font-black text-white shadow-orange transition hover:scale-105 disabled:bg-neutral-300" disabled={!available || adding}>{adding ? <span className="h-4 w-4 animate-spin rounded-full border-2 border-white border-r-transparent"/> : available ? '+' : '×'}</button>{variants.length > 1 && <span className="absolute bottom-2 left-2 rounded-full bg-white/95 px-2 py-1 text-[10px] font-black text-brand-deep shadow-sm">{variants.length} sizes</span>}</div><div className="pt-3"><div className="flex flex-wrap items-center gap-2"><b className="rounded-md bg-brand-orange px-1.5 py-1 text-sm leading-none text-white"><span className="text-[9px] text-white">KSh</span> {Number(price).toLocaleString('en-KE')}</b>{discount > 0 && <span className="text-xs font-black text-brand-deep">{discount}% off</span>}{oldPrice && <s className="text-sm text-neutral-500">{money(oldPrice)}</s>}</div><h3 className="mt-1.5 min-h-8 text-xs font-medium leading-tight text-brand-ink">{p.name}</h3><p className={`mt-0.5 text-[10px] uppercase tracking-wide ${available ? 'text-green-700' : 'text-red-600'}`}>{p.abv != null ? `${p.abv}% ABV` : 'ABV not specified'} · {available ? 'Available' : 'Out of stock'}</p></div></Link>;
}
...
export function ProductVariantCard({ product, variant }: { product: DbProduct; variant: NonNullable<DbProduct['product_variants']>[number] }) {
  ...
  return <Link href={`/product/${product.slug}?variant=${encodeURIComponent(variant.id)}`} className="block min-w-0 rounded-xl bg-white p-1.5 transition hover:-translate-y-1 hover:shadow-card"><div className="relative aspect-[3/4] overflow-hidden rounded-lg bg-white"><SmartImage src={variant.image_url || imageFor(product)} alt={`${product.name} ${variant.name} product image`} sizes="(max-width: 640px) 36vw, (max-width: 1024px) 15vw, 10vw" fit="contain" quality={95} className="scale-[1.06] transition-transform duration-300 hover:scale-110" /><button type="button" aria-label={`Add ${product.name} ${variant.name} to cart`} onClick={add} disabled={!available || adding} className="absolute right-1.5 top-1.5 grid h-8 w-8 place-items-center rounded-full bg-brand-orange text-lg font-black text-white shadow-orange transition hover:scale-105 disabled:bg-neutral-300">{adding ? <span className="h-4 w-4 animate-spin rounded-full border-2 border-white border-r-transparent"/> : available ? '+' : '×'}</button></div><div className="pt-3"><div className="flex flex-wrap items-center gap-2"><b className="rounded-md bg-brand-orange px-1.5 py-1 text-sm leading-none text-white"><span className="text-[9px] text-white">KSh</span> {Number(pricing.price).toLocaleString('en-KE')}</b>{discount > 0 && <span className="text-xs font-black text-brand-deep">{discount}% off</span>}{oldPrice && <s className="text-sm text-neutral-500">{money(oldPrice)}</s>}</div><h3 className="mt-1.5 min-h-8 text-xs font-medium leading-tight text-brand-ink">{product.name}</h3><p className={`mt-0.5 text-[10px] uppercase tracking-wide ${available ? 'text-green-700' : 'text-red-600'}`}>{product.abv != null ? `${product.abv}% ABV` : 'ABV not specified'} · {available ? 'Available' : 'Out of stock'}</p></div></Link>;
}
