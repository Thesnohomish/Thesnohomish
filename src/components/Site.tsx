...
export function Header({ content = {}, products = [] }: { content?: SiteContent; products?: DbProduct[] }) {
  ...
  return <header className="sno-header sticky top-0 z-40">
    <div className="announcement"><div className="announcement-track"><span>{announcement}</span><span aria-hidden="true">{announcement}</span></div></div>
...
export function ProductCard({ p }: { p: DbProduct }) {
  ...
  return <Link href={`/product/${p.slug}`} className="flex h-full min-w-0 flex-col rounded-xl bg-white p-1.5 transition hover:-translate-y-1 hover:shadow-card"><div className="relative"><ProductImage src={imageFor(p)} alt={`${p.name} product image`} /><button type="button" aria-label={`Add ${p.name} to cart`} onClick={add} className="absolute right-1.5 top-1.5 grid h-8 w-8 place-items-center rounded-full bg-brand-orange text-lg font-black text-white shadow-orange transition hover:scale-105 disabled:bg-neutral-300" disabled={!available || adding}>{adding ? <span className="h-4 w-4 animate-spin rounded-full border-2 border-white border-r-transparent"/> : available ? '+' : '×'}</button>{variants.length > 1 && <span className="absolute bottom-2 left-2 rounded-full bg-white/95 px-2 py-1 text-[10px] font-black text-brand-deep shadow-sm">{variants.length} sizes</span>}</div><div className="flex flex-1 flex-col pt-3"><div className="flex flex-wrap items-center gap-2"><b className="rounded-md bg-brand-orange px-1.5 py-1 text-sm leading-none text-white"><span className="text-[9px] text-white">KSh</span> {Number(price).toLocaleString('en-KE')}</b>{discount > 0 && <span className="text-xs font-black text-brand-deep">{discount}% off</span>}{oldPrice && <s className="text-sm text-neutral-500">{money(oldPrice)}</s>}</div><h3 className="mt-1.5 line-clamp-2 min-h-8 text-xs font-medium leading-tight text-brand-ink">{p.name}</h3><p className={`mt-0.5 text-[10px] uppercase tracking-wide ${available ? 'text-green-700' : 'text-red-600'}`}>{p.abv != null ? `${p.abv}% ABV` : 'ABV not specified'} · {available ? 'Available' : 'Out of stock'}</p></div></Link>;
}
...
export function ProductRail({ title, products, href, limit = 18 }: { title: string; products: DbProduct[]; href: string; limit?: number }) {
  ...
  return <section className="mx-auto max-w-none overflow-hidden px-6 py-8"><div className="mb-4 flex items-center justify-between"><h2 className="text-xl font-extrabold tracking-tight text-brand-ink">{title}</h2><Link href={href} className="font-bold text-brand-orange">View all {title}</Link></div><div className="product-rail-grid"><div className="product-rail-track" style={railStyle}><div className="product-rail-group"><CatalogCards products={products} limit={limit} /></div><div className="product-rail-group" aria-hidden="true" inert><CatalogCards products={products} limit={limit} /></div></div></div></section>;
}
