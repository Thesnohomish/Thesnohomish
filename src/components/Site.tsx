...
export function ProductRail({ title, products, href, limit = 18 }: { title: string; products: DbProduct[]; href: string; limit?: number }) {
  ...
  useEffect(()=>{if(!shouldCarousel)return;const timer=window.setInterval(()=>{const node=rail.current;if(!node||window.matchMedia('(prefers-reduced-motion: reduce)').matches)return;const atEnd=node.scrollLeft+node.clientWidth>=node.scrollWidth-8;node.scrollTo({left:atEnd?0:node.scrollLeft+node.clientWidth,behavior:'smooth'});},7000);return()=>window.clearInterval(timer)},[shouldCarousel]);
  const move = (direction:number) => rail.current?.scrollBy({left:direction*rail.current.clientWidth,behavior:'smooth'});
  return <section className="mx-auto max-w-none overflow-hidden px-8 py-8"><div className="mb-4 flex items-center justify-between"><h2 className="text-2xl font-extrabold tracking-tight text-brand-ink">{title}</h2><Link href={href} className="font-bold text-brand-orange">View all {title}</Link></div>{shouldCarousel?<div className="product-rail-frame"><button type="button" onClick={()=>move(-1)} className="product-rail-arrow left-1" aria-label={`Scroll ${title} left`}>‹</button><div ref={rail} className="product-rail-scroll"><div className="product-rail-scroll-items"><CatalogCards products={products} limit={limit}/></div></div><button type="button" onClick={()=>move(1)} className="product-rail-arrow right-1" aria-label={`Scroll ${title} right`}>›</button></div>:<div className="product-rail-static"><CatalogCards products={products} limit={9}/></div>}</section>;
}
