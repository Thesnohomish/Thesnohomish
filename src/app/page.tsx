...
export default async function Home() {
  ...
  return <main>
    ...
    <section id="wholesale" className="mx-auto grid max-w-7xl gap-4 px-5 py-16 md:grid-cols-2 md:px-8"><div className="audience-card retail category-picker"><p>Retail</p><h2>What are you after?</h2><span>Pick a shelf, start your basket.</span><div className="quick-category-grid">{quickCategories.map(category=><Link href={`/category/${category.slug}`} key={category.id}><div style={'image_url' in category&&category.image_url?{backgroundImage:`url(${category.image_url})`}:undefined}>{category.name.slice(0,2).toUpperCase()}</div><b>{category.name}</b></Link>)}</div><Link href="/shop">See all categories <ArrowRight size={17}/></Link></div><div className="audience-card wholesale"><p>Wholesale</p><Boxes/><h2>Buying for your business?</h2><span>Competitive trade pricing for bars, restaurants, hotels, supermarkets, retailers, events and corporate customers.</span><Link href="/contact?subject=trade-account">Request wholesale pricing <ArrowRight size={17}/></Link></div></section>
