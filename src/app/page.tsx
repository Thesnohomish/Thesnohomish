  ...
  return <main>
    <section className="sno-hero" style={heroImage ? {backgroundImage:`linear-gradient(90deg,rgba(5,5,5,.97) 5%,rgba(5,5,5,.79) 55%,rgba(5,5,5,.62)),url(${heroImage})`}:undefined}>
      <div className="mx-auto grid max-w-7xl gap-12 px-5 py-16 md:px-8 lg:grid-cols-[1.25fr_.75fr] lg:py-24">
        <div className="max-w-3xl"><p className="eyebrow">{heroBanner?.badge_text||'Glenbrynth whisky event'}</p><h1 className="mt-5 text-5xl font-black leading-[.98] tracking-[-.055em] text-white sm:text-6xl lg:text-7xl">{heroBanner?.title||'Buy one Glenbrynth.'}<br/><span className="text-sno-yellow">Get one free.</span></h1><p className="mt-7 max-w-2xl text-base leading-7 text-white/70 sm:text-lg">{heroBanner?.subtitle||'Explore the Glenbrynth collection—from the approachable 3YO to rich cask finishes and rare aged expressions. Limited promotional stock.'}</p><div className="mt-8 flex flex-wrap gap-3"><Link className="sno-button" href={heroBanner?.button_url||'/search?q=glenbrynth'}>{heroBanner?.button_label||heroBanner?.button_text||'Shop the offer'} <ArrowRight size={18}/></Link><Link className="sno-button-outline" href="/offers">See all deals</Link></div></div>
        <div className="glenbrynth-showcase" aria-label="Glenbrynth whisky variants">{glenbrynth.length?glenbrynth.map((product,index)=><Link href={`/product/${product.slug}`} key={product.id} className="glenbrynth-bottle" style={{'--i':index} as React.CSSProperties}>{product.image_url?<img src={product.image_url} alt={product.name}/>:<div className="bottle-placeholder"><span>GB</span></div>}<small>{product.name.replace('Glenbrynth ','')}</small></Link>):['3YO','12YO','Bourbon Cask','Sherry Cask','Rum Cask'].map((name,index)=><div className="glenbrynth-bottle" key={name} style={{'--i':index} as React.CSSProperties}><div className="bottle-placeholder"><span>GB</span></div><small>{name}</small></div>)}<strong>BUY 1<br/><span>GET 1 FREE</span></strong></div>
      </div>
    </section>
    ...
    <section id="categories" className="category-rush" aria-label="All product categories"><div className="category-rush-track">{[0,1].map(copy=><div className="category-rush-group" aria-hidden={copy===1} key={copy}>{allCategories.map((category,index)=><Link href={`/category/${category.slug}`} key={`${copy}-${category.id}`}><small>{String(index+1).padStart(2,'0')}</small><b>{category.name}</b><ArrowRight size={18}/></Link>)}</div>)}</div></section>
