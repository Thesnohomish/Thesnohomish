...
export default async function Home() {
  ...
  const categoryShowcase = quickCategories.map(category=>({category,image:('image_url' in category&&category.image_url)||products.find(product=>product.categories?.slug===category.slug)?.image_url}));
  ...
  return <main>
    ...
    <section className="alcohol-category-carousel" aria-labelledby="shop-by-category">
      ...
      <div className="alcohol-category-track">{categoryShowcase.map(({category,image})=><Link href={`/category/${category.slug}`} key={category.id}><span className="alcohol-category-image" style={image?{backgroundImage:`url(${image})`}:undefined}>{!image&&category.name.slice(0,2).toUpperCase()}</span><b>{category.name}</b></Link>)}</div>
    </section>
