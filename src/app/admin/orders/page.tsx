...
type Order = {
  ...
  total: number;
  status: string;
  delivery_location_verified?: boolean;
...
export default function OrdersPage() {
  ...
  useEffect(() => {
    ...
    if (!seenOrders.current.size) {
      seenOrders.current = new Set(orders.map((order) => order.id));
      return;
    }
    const incoming = orders.filter(
      (order) => !seenOrders.current.has(order.id),
    );
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .subscribe((state) => {
        ...
        else if (state === "CLOSED") setConnection("error");
      });
  ...
  async function changeStatus(order: Order, nextStatus: string) {
    if (
      ...
      !window.confirm(
        `${nextStatus === "rejected" ? "Reject" : "Cancel"} order ${order.order_number || order.id}?`,
      )
    )
      return;
    if (!supabase) return;
    ...
    if (updated) mergeOrder(updated);
    setNotice(
      `Order ${order.order_number || order.id} updated to ${statusLabel(nextStatus)}.`,
    );
  ...
  return (
    <main className="mx-auto max-w-7xl p-3 sm:p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="font-bold uppercase text-brand-orange">Admin</p>
          <div className="flex items-center gap-3">
            <h1 className="text-3xl font-black text-brand-ink">Orders</h1>
            {unreviewed > 0 && (
        ...
        <div className="flex items-center gap-2">
          <span
            className={`inline-flex items-center gap-2 rounded-full px-3 py-2 text-xs font-black ${connection === "live" ? "bg-green-100 text-green-800" : connection === "error" ? "bg-red-100 text-red-800" : "bg-amber-100 text-amber-800"}`}
          >
            <span
              className={`h-2 w-2 rounded-full ${connection === "live" ? "animate-pulse bg-green-600" : "bg-current"}`}
            />
            {connection === "live"
              ? "Live"
          ...
          <button
            ...
            disabled={refreshing}
            className="inline-flex items-center gap-2 rounded-xl border bg-white px-4 py-2 font-bold"
          >
      ...
      {alertOrder && (
        <aside
          role="alert"
          className="fixed inset-x-3 bottom-3 z-50 rounded-2xl border-2 border-brand-orange bg-white p-4 shadow-2xl sm:inset-x-auto sm:bottom-auto sm:right-5 sm:top-5 sm:w-96"
        >
          <button
            onClick={() => acknowledge(alertOrder.id)}
            className="absolute right-3 top-3"
            aria-label="Acknowledge new order"
          >
            <X />
          </button>
          ...
          <p className="mt-2 font-black">
            #{alertOrder.order_number || alertOrder.id.slice(0, 8)}
          </p>
          <p>
            {alertOrder.customer_name || "Guest"} · {money(alertOrder.total)}
          </p>
          <div className="mt-3 flex gap-2">
            <Link
              href={`/admin/orders/${alertOrder.id}`}
              className="rounded-xl bg-brand-orange px-4 py-2 font-black text-white"
            >
              Open order
            </Link>
            <button
      ...
      <div className="mt-5 flex flex-wrap gap-3">
        <input
          ...
          className="min-w-64 flex-1 rounded-xl border bg-white p-3"
        />
        <select
          value={status}
      ...
      {loading ? (
        ...
        <div className="mt-8 rounded-xl bg-red-50 p-4 text-red-700">
          <b>Orders could not be refreshed.</b>
          <p>{error}</p>
          <button
            onClick={() => void load(true)}
        ...
        <p className="mt-8 rounded-xl bg-white p-6 shadow-card">
