"use client";
import Link from "next/link";
import { useCallback, useEffect, useMemo, useState } from "react";
import { createBrowserSupabase } from "@/lib/supabase-browser";
import { money } from "@/lib/supabase";
export default function OrderDetail({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const supabase = useMemo(() => createBrowserSupabase(), []);
  const [id, setId] = useState(""),
    [order, setOrder] = useState<any>(null),
    [note, setNote] = useState(""),
    [rider, setRider] = useState(""),
    [phone, setPhone] = useState(""),
    [error, setError] = useState("");
  useEffect(() => {
    void params.then((x) => setId(x.id));
  }, [params]);
  const load = useCallback(async () => {
    if (!supabase || !id) return;
    const { data, error } = await supabase
      .from("orders")
      .select(
        "*,order_items(*),order_status_history(*),order_notifications(*),notification_deliveries(*)",
      )
      .eq("id", id)
      .single();
    if (error) setError(error.message);
    else {
      setOrder(data);
      setRider((current) => current || data.rider_name || "");
      setPhone((current) => current || data.rider_phone || "");
    }
  }, [supabase, id]);
  useEffect(() => {
    void load();
  }, [load]);
  useEffect(() => {
    if (!supabase || !id) return;
    const channel = supabase
      .channel(`admin-order-${id}`)
      .on(
        "postgres_changes",
        {
          event: "UPDATE",
          schema: "public",
          table: "orders",
          filter: `id=eq.${id}`,
        },
        () => void load(),
      )
      .subscribe();
    return () => {
      void supabase.removeChannel(channel);
    };
  }, [supabase, id, load]);
  async function action(status: string) {
    if (status === "out_for_delivery" && (!rider.trim() || !phone.trim())) {
      setError(
        "Add the rider name and phone number before dispatching the order.",
      );
      return;
    }
    if (
      (status === "rejected" || status === "cancelled") &&
      !confirm(`${status === "rejected" ? "Reject" : "Cancel"} this order?`)
    )
      return;
    const {
      data: { session },
    } = await supabase!.auth.getSession();
    const response = await fetch(`/api/admin/orders/${id}`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${session?.access_token || ""}`,
      },
      body: JSON.stringify({
        status,
        note,
        riderName: rider,
        riderPhone: phone,
      }),
    });
    const result = await response.json();
    if (!response.ok) setError(result.error);
    else void load();
  }
  if (!order) return <main className="p-8">{error || "Loading order…"}</main>;
  const next: any = {
    pending: ["confirmed", "rejected", "cancelled"],
    pending_payment: ["confirmed", "rejected", "cancelled"],
    paid: ["confirmed", "rejected", "cancelled"],
    accepted: ["processing", "out_for_delivery", "rejected", "cancelled"],
    confirmed: ["processing", "out_for_delivery", "rejected", "cancelled"],
    processing: ["out_for_delivery", "rejected", "cancelled"],
    dispatched: ["delivered", "cancelled"],
    out_for_delivery: ["delivered", "cancelled"],
  };
  return (
    <main className="mx-auto max-w-5xl p-5">
      <Link href="/admin/orders" className="font-bold text-brand-orange">
        ← Orders
      </Link>
      <div className="mt-4 rounded-2xl bg-white p-6 shadow-card">
        <h1 className="text-3xl font-black">
          {order.order_number || order.id}
        </h1>
        <p>
          {new Date(order.created_at).toLocaleString()} · {order.status}
        </p>
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          <div>
            <h2 className="font-black">Customer</h2>
            <p>{order.customer_name || "Guest"}</p>
            <a
              href={`tel:${order.customer_phone}`}
              className="text-brand-orange"
            >
              {order.customer_phone}
            </a>
            <p>{order.customer_email}</p>
          </div>
          <div>
            <h2 className="font-black">Delivery</h2>
            <p>{order.delivery_address}</p>
            {order.delivery_instructions && (
              <p className="text-sm text-neutral-600">
                Instructions: {order.delivery_instructions}
              </p>
            )}
            {order.delivery_location_verified === false && (
              <p className="font-bold text-red-600">Location not verified</p>
            )}
            {order.gps_lat && (
              <a
                target="_blank"
                rel="noreferrer"
                className="text-brand-orange"
                href={`https://www.google.com/maps/search/?api=1&query=${order.gps_lat},${order.gps_lng}`}
              >
                Open in Google Maps
              </a>
            )}
          </div>
        </div>
        <div className="mt-5 border-t pt-4">
          {order.order_items.map((item: any) => (
            <p key={item.id} className="flex justify-between py-1">
              <span>
                {item.quantity} × {item.product_name}
              </span>
              <b>{money(item.line_total)}</b>
            </p>
          ))}
          <p className="mt-3 flex justify-between">
            Subtotal <b>{money(order.subtotal)}</b>
          </p>
          <p className="flex justify-between">
            Discount <b>{money(order.discount_total)}</b>
          </p>
          <p className="flex justify-between">
            Delivery <b>{money(order.delivery_fee)}</b>
          </p>
          <p className="flex justify-between text-xl font-black">
            Total <b>{money(order.total)}</b>
          </p>
        </div>
        <p className="mt-4">
          Payment:{" "}
          <b>
            {order.payment_method} · {order.payment_status}
          </b>
        </p>
        {order.gift_note && (
          <p className="mt-2">Gift note: {order.gift_note}</p>
        )}
        {["accepted", "confirmed", "processing"].includes(order.status) && (
          <section className="mt-5 rounded-2xl border-2 border-brand-orange bg-orange-50 p-4">
            <h2 className="text-lg font-black text-brand-ink">Rider details</h2>
            <p className="mt-1 text-sm text-neutral-700">
              Add both details before clicking <strong>Dispatch order</strong>.
              The customer will receive them in the out-for-delivery email.
            </p>
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              <label className="font-bold">
                Rider name
                <input
                  value={rider}
                  onChange={(event) => setRider(event.target.value)}
                  className="mt-1 w-full rounded-xl border bg-white p-3 font-normal"
                  placeholder="Enter rider's full name"
                  autoComplete="name"
                />
              </label>
              <label className="font-bold">
                Rider phone number
                <input
                  value={phone}
                  onChange={(event) => setPhone(event.target.value)}
                  className="mt-1 w-full rounded-xl border bg-white p-3 font-normal"
                  placeholder="e.g. 0712 345 678"
                  inputMode="tel"
                  autoComplete="tel"
                />
              </label>
            </div>
          </section>
        )}
        <textarea
          value={note}
          onChange={(e) => setNote(e.target.value)}
          className="mt-5 w-full rounded-xl border p-3"
          placeholder="Optional status note"
        />
        <div className="mt-5 flex flex-wrap gap-3">
          {(next[order.status] || []).map((status: string) => (
            <button
              key={status}
              onClick={() => void action(status)}
              className="rounded-xl bg-brand-orange px-4 py-3 font-black text-white"
            >
              {status === "confirmed"
                ? "Confirm order"
                : status === "processing"
                  ? "Start processing"
                  : status === "out_for_delivery"
                    ? "Dispatch order"
                    : status === "delivered"
                      ? "Mark delivered"
                      : status === "cancelled"
                        ? "Cancel order"
                        : "Reject order"}
            </button>
          ))}
        </div>
        {error && <p className="mt-3 text-red-600">{error}</p>}
      </div>
      <section className="mt-5 rounded-2xl bg-white p-6 shadow-card">
        <h2 className="text-xl font-black">Status & notification history</h2>
        {order.order_status_history?.map((entry: any) => (
          <p key={entry.id} className="border-b py-2">
            {entry.from_status || "New"} → {entry.to_status} ·{" "}
            {new Date(entry.created_at).toLocaleString()}
          </p>
        ))}
        {order.notification_deliveries?.map((entry: any) => (
          <p key={entry.id} className="border-b py-2">
            {entry.channel === "email" ? "Email" : entry.channel}:{" "}
            {entry.status === "sent"
              ? "Sent"
              : entry.status === "failed"
                ? "Send failed"
                : "Pending"}{" "}
            {entry.error_message && `— ${entry.error_message}`}
          </p>
        ))}
        {order.order_notifications
          ?.filter((entry: any) => entry.channel !== "email")
          .map((entry: any) => (
            <p key={entry.id} className="border-b py-2">
              {entry.channel}: {entry.status}{" "}
              {entry.error_message && `— ${entry.error_message}`}
            </p>
          ))}
      </section>
    </main>
  );
}
