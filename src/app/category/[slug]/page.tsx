...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const description = truncate(category?.seo_description || plainText(category?.description) || `Order ${name.toLowerCase()} online from The Snohomish with fast, reliable ${name.toLowerCase()} delivery across Nairobi.`);
  ...
  return {
    ...
    openGraph: { title: `${title} | The Snohomish`, description, url, type: 'website', images: category?.image_url ? [{ url: category.image_url, alt: `${name} delivery Nairobi` }] : undefined },
    twitter: { card: category?.image_url ? 'summary_large_image' : 'summary', title: `${title} | The Snohomish`, description, images: category?.image_url ? [category.image_url] : undefined },
  };
}
