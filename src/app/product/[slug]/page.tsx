...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const description = truncate(product.seo_description || plainText(product.short_description || product.description) || `Order ${product.name} online from The Snohomish for fast alcohol delivery across Nairobi.`);
  ...
  return {
    ...
    openGraph: { title: `${title} | The Snohomish`, description, url, type: 'website', images: images.map((image) => ({ url: image, alt: product.name })) },
    twitter: { card: 'summary_large_image', title: `${title} | The Snohomish`, description, images: images.slice(0, 1) },
  };
}
...
export default async function Product({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ variant?: string }> }) {
  ...
  return <main className="mx-auto max-w-6xl px-4 py-4 sm:py-6">
