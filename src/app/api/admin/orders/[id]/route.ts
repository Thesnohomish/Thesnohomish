...
// Customers receive status email only when the order actually leaves with a rider.
// Checkout separately sends the initial `placed` email.
const emailEventForStatus: Partial<Record<string, OrderEmailEvent>> = { out_for_delivery: 'dispatched' };
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.status === 'out_for_delivery' && (!riderName || !riderPhone)) return NextResponse.json({ error: 'Add the rider name and phone number before dispatching the order.' }, { status: 400 });
  const update: Record<string, unknown> = { status: body.status, updated_at: new Date().toISOString() }; if (body.status === 'out_for_delivery') Object.assign(update, { dispatched_at: new Date().toISOString(), rider_name: riderName, rider_phone: riderPhone, delivery_note: body.deliveryNote || null, tracking_url: body.trackingUrl || null });
  ...
  if (emailEvent && order.customer_email) {
    const emailOrder: EmailOrder = { id: order.id, orderNumber: order.order_number, customerName: order.customer_name || 'Customer', customerEmail: order.customer_email, customerPhone: order.customer_phone, deliveryAddress: order.delivery_address || 'Delivery address on order', paymentMethod: order.payment_method, subtotal: Number(order.subtotal), deliveryFee: Number(order.delivery_fee), total: Number(order.total), estimatedDelivery: 'Out for delivery', riderName, riderPhone, items: (order.order_items || []).map(item => ({ name: item.product_name, quantity: Number(item.quantity), unitPrice: Number(item.unit_price), lineTotal: Number(item.line_total) })) };
    await sendOrderEmail(admin.db, emailOrder, emailEvent, order.customer_email);
  }
  if (body.status === 'dispatched') { const text = `Your The Snohomish order #${order.order_number} has been dispatched and is on its way. Order total: KES ${Number(order.total).toLocaleString('en-KE')}. Rider: ${body.riderName || 'The Snohomish rider'}. Contact: ${body.riderPhone || 'Contact The Snohomish'}. Thank you for shopping with The Snohomish.`; if (order.customer_phone) await admin.db.from('order_notifications').upsert([{ order_id: id, channel: 'sms', recipient: order.customer_phone, event_key: 'dispatched', status: 'not_configured', error_message: 'SMS provider not configured' }, { order_id: id, channel: 'whatsapp', recipient: order.customer_phone, event_key: 'dispatched', status: 'not_configured', error_message: 'WhatsApp provider not configured' }], { onConflict: 'order_id,channel,recipient,event_key', ignoreDuplicates: true }); await admin.db.from('admin_notifications').insert({ order_id: id, kind: 'dispatch', title: `Order ${order.order_number} dispatched`, body: text }); }
