...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: payment } = await db.from('payments').select('id,order_id,status,amount').eq('checkout_request_id', callback.CheckoutRequestID).maybeSingle();
    if (!payment || payment.status === 'paid') return NextResponse.json({ ResultCode: 0, ResultDesc: 'Accepted' });

    const metadata = Object.fromEntries((callback.CallbackMetadata?.Item || []).map((item: { Name: string; Value?: unknown }) => [item.Name, item.Value]));
    const success = Number(callback.ResultCode) === 0 && Number(metadata.Amount) === Number(payment.amount);
    const paymentStatus = success ? 'paid' : Number(callback.ResultCode) === 1032 ? 'cancelled' : Number(callback.ResultCode) === 1037 ? 'timed_out' : 'failed';

    await db.from('payments').update({
      status: paymentStatus,
      receipt_number: String(metadata.MpesaReceiptNumber || '') || null,
      transaction_at: metadata.TransactionDate ? new Date(String(metadata.TransactionDate)).toISOString() : null,
    ...
    await db.from('orders').update({
      ...
      status: success ? 'paid' : 'awaiting_payment',
    }).eq('id', payment.order_id).eq('payment_status', 'pending_payment');

    if (success) {
      const { data: order } = await db.from('orders').select('id,order_number,customer_name,customer_email,customer_phone,delivery_address,payment_method,subtotal,delivery_fee,total,order_items(product_name,quantity,unit_price,line_total)').eq('id', payment.order_id).maybeSingle();
      if (order) {
