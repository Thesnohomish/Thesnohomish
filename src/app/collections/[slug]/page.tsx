...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const section = builtIn ? null : await getHomepageSection(slug);
  const title = builtIn?.title || section?.heading || 'Collection';
  const description = section?.description || `Shop ${title.toLowerCase()} from The Snohomish with convenient drinks delivery across Nairobi.`;
  return { title, description, alternates: { canonical: `/collections/${slug}` }, openGraph: { title: `${title} | The Snohomish`, description, url: `/collections/${slug}`, type: 'website' }, twitter: { card: 'summary', title: `${title} | The Snohomish`, description }, robots: builtIn ? { index: true, follow: true } : { index: false, follow: false } };
}
...
export default async function CollectionPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (customSection) {
    ...
    products = customSection.product_ids.map(id => allProducts.find(product => product.id === id)).filter((product): product is Product => Boolean(product));
  } else {
